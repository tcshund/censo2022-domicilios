...
domicilios <- read_parquet(
  ...
  mutate(
      ...
      dom04_dccm_uni_0 = V01196, # unidade em dccm
      # moradores em dccm
...
cn22dom_vars <- cn22dom_vars %>%
  select(
    all_of(id_cols),
    matches("_(0|1|2)$")  # 0 = absoluto simples, 1 = absoluto composto, 2 = relativo
  )
...
```{r}
#| label: check-id-cols-vars 
#| echo: false 
id_presentes <- sum(id_cols %in% names(cn22dom_vars))
cat("- IDs:", id_presentes, "/", length(id_cols), "OK\n")
```

#### Integridade dos dados numéricos

```{r}
#| label: check-dados-vars 
#| echo: false 
vars_num <- names(cn22dom_vars)[!names(cn22dom_vars) %in% id_cols]

conta_problemas <- function(x) {
  c(
    "NA"     = sum(is.na(x) & !is.nan(x)),
    "NaN"    = sum(is.nan(x)),
...
problemas_tipo <- t(sapply(cn22dom_vars[vars_num], conta_problemas))

total_problemas <- sum(problemas_tipo)

cat(
  "- Dados numéricos:",
  ifelse(total_problemas == 0,
         "OK",
         paste("AVISO:", total_problemas, "problemas encontrados")),
  "\n"
)
```
